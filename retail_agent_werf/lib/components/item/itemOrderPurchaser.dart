...
class ItemOrderPurchaser extends StatefulWidget {
  final OrderModelPurchaser orderModel;
  const ItemOrderPurchaser({Key? key, required this.orderModel})
      : super(key: key);

  @override
  _ItemOrderPurchaserState createState() => _ItemOrderPurchaserState();
}

class _ItemOrderPurchaserState extends State<ItemOrderPurchaser> {
  @override
  Widget build(BuildContext context) {
    return Container(
      height: 111,
      padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 20),
      margin: EdgeInsets.only(top: 6, left: 20, right: 20, bottom: 6),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                _textStatus(widget.orderModel.status),
                SizedBox(
                  height: 10,
                ),
                Container(
                  ...
                  child: CircleAvatar(
                    ...
                    backgroundImage: AssetImage(
                      'assets/images/${widget.orderModel.img}',
                    ),
                  ),
                ),
              ],
